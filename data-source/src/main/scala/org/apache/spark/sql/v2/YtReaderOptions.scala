...
import tech.ytsaurus.spyt.format.conf.{SparkYtConfiguration, SparkYtInternalConfiguration, YtTableSparkSettings}
...
object YtReaderOptions {
  ...
  def fullReadAllowed(options: Map[String, String]): Boolean = {
    import tech.ytsaurus.spyt.wrapper.config._
    options.getYtConf(SparkYtInternalConfiguration.FullReadAllowed).exists(identity)
  }

  def canReadBatch(dataSchema: StructType, optimizedForScan: Boolean,
    fullReadAllowed: Boolean, arrowEnabled: Boolean): Boolean = {
    (optimizedForScan && arrowEnabled && arrowSchemaSupported(dataSchema) && fullReadAllowed) || dataSchema.isEmpty
  }

  def canReadBatch(dataSchema: StructType, options: Map[String, String], conf: SQLConf): Boolean = {
    canReadBatch(dataSchema, optimizedForScan(options), fullReadAllowed(options), arrowEnabled(options, conf))
  }
