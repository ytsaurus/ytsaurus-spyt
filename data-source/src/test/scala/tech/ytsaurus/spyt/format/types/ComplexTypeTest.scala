...
class ComplexTypeTest extends FlatSpec with Matchers with LocalSpark with TmpDir with TestUtils {
  ...
  it should "read decimal with schema hint" in {
    val precision = 3
    val scale = 2
    val data = Seq("1.23", "0.21")
    val byteDecimal = data.map(x => textToBinary(x, precision, scale))
    byteDecimal.toDF("a").write.yt(tmpPath)

    val res = spark.read.schemaHint(("a", DecimalType(precision, scale))).yt(tmpPath)
  ...
  it should "not write dataset with complex types" in {
    import spark.implicits._
    ...
    try {
      ...
      case t: Throwable =>
        val cause = Throwables.getRootCause(t)
        if (
          !cause.isInstanceOf[IllegalStateException]
            || cause.asInstanceOf[IllegalStateException].getMessage != "unable to serialize a complex type: use spark.yt.write.typeV3.enabled"
        ) {
          throw t
        }
    }
