...
private[spark] class SparkSubmitSpyt {
  ...
  : (Seq[String], Seq[String], SparkConf, String) = {
      ...
      if (!Utils.classIsLoadable(YTSAURUS_CLUSTER_SUBMIT_CLASS) && !Utils.isTesting) {
        error(
          "Could not load YTSAURUS classes. " +
            ...
            "1. Install ytsaurus-spyt python package via \u001b[1mpip install ytsaurus-spyt\u001b[0m\n" +
            "2. Activate SPYT configuration in environment by running " +
            "\u001b[1msource spyt-env\u001b[0m command\n")
      }
    ...
    val options = List[OptionAssigner](
      ...
      OptionAssigner(args.ivyRepoPath, YTSAURUS, CLUSTER, confKey = "spark.jars.ivy"),
...
object SparkSubmitSpyt {
  ...
  private[deploy] def processOptions(
    options: List[OptionAssigner],
    deployMode: Int,
    clusterManager: Int,
    sparkConf: SparkConf): ArrayBuffer[String] = {
    val childArgs = new ArrayBuffer[String]()
    for (opt <- options) {
      if (opt.value != null &&
...
object SparkSubmitDecorators {
  ...
  @DecoratedMethod
  def org$apache$spark$deploy$SparkSubmit$$ALL_CLUSTER_MGRS(): Int = {
    __org$apache$spark$deploy$SparkSubmit$$ALL_CLUSTER_MGRS() | YTsaurusConstants.YTSAURUS
  }


  private[deploy] def __isPython(res: String): Boolean = ???
  def __org$apache$spark$deploy$SparkSubmit$$ALL_CLUSTER_MGRS(): Int = ???
}
...
class ClusterManagerInitializerBytecodeModifier extends MethodProcesor {
  ...
  private def addYTsaurusClause(cp: ConstPool, ca: CodeAttribute): Unit = {
    ...
    found = false
    var insertPos = 0
    val isScala213 = scala.util.Properties.versionNumberString.startsWith("2.13")
    while (ci.hasNext && !found) {
      val index = ci.next()
      val op = ci.byteAt(index)
      // found goto pos+3 for scala 2.12 or first goto for scala 2.13
      if (op == Opcode.GOTO && (isScala213 || ci.u16bitAt(index + 1) == 3)) {
        found = true
