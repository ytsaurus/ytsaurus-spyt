...
import org.apache.spark.deploy.master.MasterDecorators.javaOpts
...
class MasterDecorators {
  ...
  def org$apache$spark$deploy$master$Master$$createDriver(desc: DriverDescription): DriverInfo = {
    ...
    val newDesc = dInfo.desc.copy(command =
      desc.command.copy(javaOpts = javaOpts(dInfo.id, desc.command.javaOpts)))
...
object MasterDecorators extends Logging {
  ...
  def javaOpts(driverId: String, baseOpts: Seq[String]): Seq[String] = s"-Dspark.driverId=$driverId" +: baseOpts
}