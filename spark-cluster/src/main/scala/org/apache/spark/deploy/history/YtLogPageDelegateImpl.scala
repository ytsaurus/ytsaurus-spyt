...
class YtLogPageDelegateImpl(conf: SparkConf) extends YtLogPageDelegate with Logging {
  ...
  override def renderLog(getParameter: String => String): String = {
    val (app, exec, logType, _, _, _, logText, startRow, endRow, logLength) = getLogQuery(getParameter)
    val pre = s"==== Rows $startRow-$endRow of $logLength of $app/$exec/$logType ====\n"
    pre + logText
  }

  override def render(getParameter: String => String): (Seq[Node], String) = {
    val (_, _, logType, rowLength, params, pageName, logText, startRow, endRow, logLength) = getLogQuery(getParameter)
    ...
    (content, logType + " log page for " + pageName)
  }

  private def getLogQuery(getParameter: String => String):
    (String, String, String, Int, String, String, String, Long, Long, Long) = {
    val appId = Option(getParameter("appId"))
    val executorId = Option(getParameter("executorId"))
    val driverId = Option(getParameter("driverId"))
    val logType = getParameter("logType")
    val offset = Option(getParameter("offset")).map(_.toLong)
    val rowLength = Option(getParameter("rowLength")).map(_.toInt)
