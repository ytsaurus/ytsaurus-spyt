...
class SpytEnablers(object):
    ...
    YT_LOGS_KEY = "spark.ytsaurus.logs.export.enabled"
    ...
    def __init__(self, enable_profiling=False, enable_arrow=True,
                 enable_mtn=False, enable_yt_metrics=True, enable_monium_logs_export=False, enable_preference_ipv6=True,
        ...
        self.enable_task_proxy = enable_task_proxy
    ...
    def apply_config(self, config):
        ...
        self.enable_yt_metrics = self._get_enabler(self.enable_yt_metrics, "enable_solomon_agent",
                                                   self.YT_METRICS_KEY)
        self.enable_monium_logs_export = self._get_enabler(self.enable_monium_logs_export, "enable_monium_logs_export", self.YT_LOGS_KEY)
        self.enable_tcp_proxy = self._get_enabler(self.enable_tcp_proxy, "enable_tcp_proxy", self.TCP_PROXY_KEY)
        if self.enable_task_proxy is None and self.TASK_PROXY_KEY in spark_conf:
            self.enable_task_proxy = parse_bool(spark_conf[self.TASK_PROXY_KEY])
        self.enable_task_proxy = self._get_enabler(self.enable_task_proxy, "enable_task_proxy", self.TASK_PROXY_KEY)
